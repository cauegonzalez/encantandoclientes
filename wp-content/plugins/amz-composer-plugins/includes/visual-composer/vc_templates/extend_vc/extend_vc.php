...
if( defined( 'COMPOSER_EXTRAS_URI' ) ) {
...
vc_map( array(
	...
	"params" => array(
		...
		array(
			...
			"value" => "5px",
			"description" => __("Enter thickness in Pixels (eg: 4px), Leave empty to apply default 5px", 'amz-composer-plugins'),
             "dependency" => array('element' => "line_style", 'value' => array('line-style1'))
			),
...
$post_types = array_merge( array( 'post' => 'post' ), $custom_post_types );
$exclude_post_types = array( 'forum', 'topic', 'reply' );

$post_types = array_diff( $post_types, $exclude_post_types );

foreach ( $post_types as $key => $post_type ) {
	$obj = get_post_type_object( $post_type );
	$post_type_list[$obj->label] = $key;
}

$post_type_list['Portfolio'] = 'pix_portfolio';
$post_type_list = apply_filters( 'composer_all_post_types_lists', $post_type_list );

$taxonomy_list = array(); 

$all_taxonomies = get_taxonomies();
$exclude_taxonomies = array( 'post_tag', 'nav_menu', 'link_category', 'post_format', 'topic-tag' );

if( defined( 'WC_VERSION' ) ) {

	$exclude_taxonomies = array( 'post_tag', 'nav_menu', 'link_category', 'post_format', 'topic-tag', 'product_type', 'product_visibility', 'product_tag', 'product_shipping_class' );

	$attributes = wc_get_attribute_taxonomies();

	foreach( $attributes as $key => $attr ) {
		$exclude_taxonomies[] = 'pa_'.$attr->attribute_name;
	}
}

$taxonomies = array_diff( $all_taxonomies, $exclude_taxonomies );

foreach ( $taxonomies as $taxonomy ) {
	$taxonomy_obj = get_taxonomy( $taxonomy );
    $taxonomy_list[$taxonomy_obj->label] = $taxonomy;
}

$taxonomy_list['Portfolio Category'] = 'pix_categories';
$taxonomy_list = apply_filters( 'composer_all_taxonomy_lists', $taxonomy_list );
...
vc_map( array(
	...
	"params" => array(
		...
		array(
			"type" => "dropdown",
			"heading" => __("Post Types", 'amz-composer-plugins'),
			"param_name" => "post_type",
			"value" => $post_type_list,
			"description" => __("Choose the post types", 'amz-composer-plugins'),
		),

		array(
			"type" => "dropdown",
			"heading" => __( "Taxonomy", 'amz-composer-plugins' ),
			"param_name" => "taxonomy",
			"value" => $taxonomy_list,
			"description" => 'Choose the taxonomy. Default: Categories( Blog Category )'
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Style', 'amz-composer-plugins' ),
			'param_name' => 'style',
			'value' => array( 
				esc_html__( 'Normal', 'amz-composer-plugins' )            => 'normal',
				esc_html__( 'Overlay', 'amz-composer-plugins' )           => 'overlay',
				esc_html__( 'Grey Box', 'amz-composer-plugins' )          => 'grey_box',
				esc_html__( 'Static Background', 'amz-composer-plugins' ) => 'static_bg',
				esc_html__( 'Simple', 'amz-composer-plugins' )            => 'simple',
				esc_html__( 'Modern', 'amz-composer-plugins' )            => 'modern',
				esc_html__( 'Slide', 'amz-composer-plugins' )             => 'slide',
				esc_html__( 'Masonry', 'amz-composer-plugins' )           => 'masonry',
				esc_html__( 'Grid', 'amz-composer-plugins' )              => 'grid'
			),
			'description' => esc_html__( 'Choose the blog style', 'amz-composer-plugins' ),
		),

		array(
			"type" => "dropdown",
			"heading" => __("Blog slide style post count", 'amz-composer-plugins'),
			"param_name" => "slide_style_pos",
			"value" => array(
				__('1', 'amz-composer-plugins') => "1",
				__('2', 'amz-composer-plugins') => "2",
				__('3', 'amz-composer-plugins') => "3",
				__('4', 'amz-composer-plugins') => "4"
			),
			"description" => 'Set the default open post count',
			"dependency" => array('element' => "style", 'value' => array('slide')),
		),
		...
		array(
			"type" => "dropdown",
			"heading" => __("Insert Post By", 'amz-composer-plugins'),
			"param_name" => "insert_type",
			"admin_label" => true,
			"value" => array(
				__('Posts', 'amz-composer-plugins') => "posts", 
				__('Post Id', 'amz-composer-plugins') => "id",
				__('Post Category', 'amz-composer-plugins') => "category"),
			"description" => ''
			),
		...
		array(
			...
			"value" => array('h3','h1','h2','h4','h5','h6','p'),
		...
		array(
			"type" => "textfield",
			"heading" => __("Image Width", 'amz-composer-plugins'),
			"param_name" => "image_width",
			"value" => "",
			"description" => __("Type the image width", 'amz-composer-plugins')
		),

		array(
			"type" => "textfield",
			"heading" => __("Image Height", 'amz-composer-plugins'),
			"param_name" => "image_height",
			"value" => "",
			"description" => __("Type the image height", 'amz-composer-plugins')
		),
		...
		array(
			"type" => "dropdown",
			"heading" => __("Above Image", 'amz-composer-plugins'),
			"param_name" => "above_image_meta",
			"value" => array(
				__('None', 'amz-composer-plugins') => "none",
				__('Date', 'amz-composer-plugins') => "date",
		...
		array(
			"type" => "dropdown",
			"heading" => esc_html__("Pagination", 'amz-composer-plugins'),
			"param_name" => "blog_pagination",
			"value" => array(
				esc_html__( 'None', 'amz-composer-plugins')      => "none", 
				esc_html__('Load More', 'amz-composer-plugins') => "load_more", 
				esc_html__('Autoload', 'amz-composer-plugins')  => "autoload", 
				esc_html__('Number', 'amz-composer-plugins')    => "number", 
		...
		array(
			"type" => "textfield",
			"heading" => esc_html__("Loadmore Text", 'amz-composer-plugins'),
			"param_name" => "loadmore_text",
			"value" => esc_html__( "Load More", 'amz-composer-plugins' ),
			"description" => esc_html__("Type the load more text", 'amz-composer-plugins'),
			"dependency" => array('element' => "blog_pagination", 'value' => array('load_more'))
		),

		array(
			"type" => "textfield",
			"heading" => esc_html__("All Post Loaded Text", 'amz-composer-plugins'),
			"param_name" => "allpost_loaded_text",
			"value" => esc_html__( "All Post Loaded", 'amz-composer-plugins' ),
			"description" => esc_html__("Type the all post loaded text", 'amz-composer-plugins'),
			"dependency" => array('element' => "blog_pagination", 'value' => array('load_more'))
		),

		array(
			"type" => "dropdown",
			"heading" => esc_html__("Change Url", 'amz-composer-plugins'),
			"param_name" => "change_url",
			"value" => array(
				esc_html__('No', 'amz-composer-plugins') => "no",
				esc_html__('Yes', 'amz-composer-plugins') => "yes"
			),
			"description" => esc_html__("Do you want to change url in address bar on load more/autoload option", 'amz-composer-plugins'),
			"dependency" => array('element' => "pagination", 'value' => array('load_more','autoload'))
		),
...
vc_map( array(
	...
	"params" => array(
		...
		array(
			"type" => "dropdown",
			"heading" => esc_html__( "Category in thumb", 'amz-composer-plugins' ),
			"param_name" => "category_in_thumb",
			"value" => array(
				esc_html__('Show', 'amz-composer-plugins' ) => "show",							 
				esc_html__('Hide', 'amz-composer-plugins' ) => "hide"
			),
			"description" => esc_html__( 'Do you want to show category in thumbnail?', 'amz-composer-plugins' )
		),
		...
		array(
			...
			"description" => __("Do you want to display description", 'amz-composer-plugins')
		),

		array(
			"type" => "textfield",
			"heading" => __("Description Length", 'amz-composer-plugins'),
			"param_name" => "desc_length",
			"value" => "20",
			"description" => __("Type the integer value", 'amz-composer-plugins'),
			"dependency" => array('element' => "show_description", 'value' => array('yes'))
		),
		...
		array(
			...
			"description" => '',
			"dependency" => array('element' => "insert_type", 'value' => array('posts')),
		),
		...
		array(
			"type" => "dropdown",
			"heading" => __("Grid Animation", 'amz-composer-plugins'),
			"param_name" => "grid_animation",
			"value" => array( __('Yes', 'amz-composer-plugins') => "enable", 
							  __('No', 'amz-composer-plugins') => "disable" ),
			"description" => __("Enable grid animation?", 'amz-composer-plugins'),
			"group"	=> __('Grid Animation', 'amz-composer-plugins')
		),

		array(
			"type" => "dropdown",
			"heading" => __("Animation Transition", 'amz-composer-plugins'),
			"param_name" => "grid_transition",
			"value" => $animation,
			"description" => __("Choose Animation Transition.", 'amz-composer-plugins'),
			"dependency" => array('element' => "grid_animation", 'value' => array('enable')),
			"group"	=> __('Grid Animation', 'amz-composer-plugins')
			),
		...
		array(
			"type" => "textfield",
			"heading" => __("Animation Delay", 'amz-composer-plugins'),
			"param_name" => "grid_delay",
			"value" => "200",
			"description" => __("Enter the Delay in millisecond (Ex: 100)", 'amz-composer-plugins'),
			"dependency" => array('element' => "grid_animation", 'value' => array('enable')),
			"group"	=> __('Grid Animation', 'amz-composer-plugins')
			),
		)
...
vc_map( array(
	...
	"params" => array(
		...
		array(
			"type" => "textfield",
			"heading" => __("Instagram Url", 'amz-composer-plugins'),
			"param_name" => "instagrem",
			"value" => "",
			"description" => __("Enter the instagram Url", 'amz-composer-plugins')
			),
		...
		array(
			"type" => "textfield",
			"heading" => __("Blogger Url", 'amz-composer-plugins'),
			"param_name" => "blogger",
			"value" => "",
...
// Breadcrumbs
vc_map( array(
	"name" => esc_html__( 'Breacrumbs', 'amz-composer-plugins' ), //Title
	"base" => 'composer_breadcrumbs_sc',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'By Composer', 'amz-composer-plugins' ), 
	"params" => array(

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Extra class', 'amz-composer-plugins' ),
			'param_name' => 'el_class',
			'value' => '',
			'description' => esc_html__( 'If you wish to style particular content element differently, then use this field to add a class name and then refer to it in your css file.', 'amz-composer-plugins' )
		),
...
// Modern Post
vc_map( array(
	'name' => esc_html__( 'Modern Post', 'amz-composer-plugins' ), //Title
	'base' => 'modern_posts',
	'class' => '',
	'icon' => 'icon-pixel8es',
	'category' => esc_html__( 'By Composer', 'amz-composer-plugins' ), 
	'params' => array(
		...
		array(
			...
			'description' => esc_html__( 'Choose the style', 'amz-composer-plugins' ),
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Post Types', 'amz-composer-plugins' ),
			'param_name' => 'post_type',
			'value' => array(
				esc_html__( 'Portfolio', 'composer' ) => 'pix_portfolio',
				esc_html__( 'Posts', 'composer' )     => 'post',
				esc_html__( 'Products', 'composer' )     => 'product'
			),
			'description' => esc_html__( 'Choose the post types', 'amz-composer-plugins' ),
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Order By', 'amz-composer-plugins' ),
			'param_name' => 'order_by',
			'value' => array( 
				esc_html__( 'Date Modified', 'amz-composer-plugins' ) => 'modified',
				esc_html__( 'Date', 'amz-composer-plugins' ) => 'date',  
				esc_html__( 'Rand', 'amz-composer-plugins' ) => 'rand',
				esc_html__( 'ID', 'amz-composer-plugins' ) => 'ID', 
				esc_html__( 'Title', 'amz-composer-plugins' ) => 'title', 
				esc_html__( 'Author', 'amz-composer-plugins' ) => 'author',
				esc_html__( 'Name', 'amz-composer-plugins' ) => 'name',
				esc_html__( 'Parent', 'amz-composer-plugins' ) => 'parent',							  
				esc_html__( 'Menu Order', 'amz-composer-plugins' ) => 'menu_order',
				esc_html__( 'None', 'amz-composer-plugins' ) => 'none'
			),
			'description' => esc_html__( 'Order posts By choosen order', 'amz-composer-plugins' )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Order', 'amz-composer-plugins' ),
			'param_name' => 'order',
			'value' => array( 
				esc_html__( 'Descending order', 'amz-composer-plugins' ) => 'desc', 
				esc_html__( 'Ascending order', 'amz-composer-plugins' ) => 'asc' 
			),
			'description' => esc_html__( 'In which Order, you want to display posts', 'amz-composer-plugins' )
		),
		...
		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Title length', 'amz-composer-plugins' ),
			'param_name' => 'title_length',
			'value' => '30',
			'description' => esc_html__( 'Enter excerpt length. Example: 180 (Leave it empty to apply default)', 'amz-composer-plugins' )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Content Position', 'amz-composer-plugins' ),
			'param_name' => 'content_position',
			'value' => array( 
				esc_html__( 'Image on Left', 'amz-composer-plugins' ) => 'image-left', 
				esc_html__( 'Content on Left', 'amz-composer-plugins' ) => 'content-left' 
			),
			'description' => esc_html__( 'In which position, you want to display posts content and image', 'amz-composer-plugins' )
		),
		...
		array(
			...
			'heading' => esc_html__( 'Title', 'amz-composer-plugins' ),
			'param_name' => 'title',
			'value' => esc_html__( 'Selected Works', 'amz-composer-plugins' ),
			'description' => esc_html__( 'Enter the title', 'amz-composer-plugins' )
		),
...
// Horizontal Post
/*vc_map( array(
	'name' => esc_html__( 'Horizontal Post Overlay', 'amz-composer-plugins' ), //Title
	'base' => 'horizontal_post_overlay',
	'class' => '',
	'icon' => 'icon-pixel8es',
	'category' => esc_html__( 'By Composer', 'amz-composer-plugins' ), 
	'params' => array(
		...
		array(
			"type" => "textfield",
			"heading" => esc_html__("Post Count", 'amz-composer-plugins'),
			"param_name" => "items",
			"value" => 6,
			"dependency" => array('element' => "insert_type", 'value' => array('posts','category')),
			"description" => esc_html__("Enter the number of Posts you want to display (only numbers), Enter -1 for all posts", 'amz-composer-plugins')
			),

		array(
			"type" => "dropdown",
			"heading" => esc_html__("Insert Post By", 'amz-composer-plugins'),
			"param_name" => "insert_type",
			"admin_label" => true,
			"value" => array(
				esc_html__('Posts', 'amz-composer-plugins') => "posts", 
				esc_html__('Post Id', 'amz-composer-plugins') => "id",
				esc_html__('Post Category', 'amz-composer-plugins') => "category"
			),
			"description" => ''
			),
		...
		array(
			"type" => "textfield",
			"heading" => esc_html__("Post Id", 'amz-composer-plugins'),
			"param_name" => "id",
			"value" => "",
			"dependency" => array('element' => "insert_type", 'value' => 'id'),
			"description" => esc_html__("Enter the blog post ids seperated by commas (,). Example: 2,54,8", 'amz-composer-plugins')
			),

		array(
			"type" => "textfield",
			"heading" => esc_html__("Category/Terms", 'amz-composer-plugins'),
			"param_name" => "category",
			"value" => "",
			"dependency" => array('element' => "insert_type", 'value' => 'category'),
			"description" => esc_html__("Enter the blog post category seperated by commas (,). Example: design,illustration,print", 'amz-composer-plugins')
			),

		array(
			"type" => "textfield",
			"heading" => esc_html__("Exclude Post ID", 'amz-composer-plugins'),
			"param_name" => "exclude_id",
			"value" => "",
			"dependency" => array('element' => "insert_type", 'value' => array('posts','category')),
			"description" => esc_html__("Enter the exclude blog post ids seperated by commas (,). Example: 2,54,8", 'amz-composer-plugins')
			),

		array(
			"type" => "textfield",
			"heading" => esc_html__("Exclude Category", 'amz-composer-plugins'),
			"param_name" => "exclude_category",
			"value" => "",
			"dependency" => array('element' => "insert_type", 'value' => array('posts','category')),
			"description" => esc_html__("Enter the blog post category seperated by commas (,). Example: design,illustration,print", 'amz-composer-plugins')
		...
		array(
			"type" => "dropdown",
			"heading" => esc_html__( "Title Tag", 'amz-composer-plugins' ),
			"param_name" => "title_tag",
			"value" => array('h3','h1','h2','h4','h5','h6','p'),
			"description" => esc_html__("Select title tag.", 'amz-composer-plugins')
		),
		...
		array(
			...
			'param_name' => 'content_position',
			'value' => array( 
				esc_html__( 'Top Left', 'amz-composer-plugins' )     => 'top-left', 
				esc_html__( 'Top Right', 'amz-composer-plugins' )    => 'top-right', 
				esc_html__( 'Bottom Left', 'amz-composer-plugins' )  => 'bottom-left',
				esc_html__( 'Bottom Right', 'amz-composer-plugins' ) => 'bottom-right',
				esc_html__( 'Center', 'amz-composer-plugins' )       => 'center' 
			),
			'description' => esc_html__( 'In which position, you want to display posts content', 'amz-composer-plugins' )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Show Category', 'amz-composer-plugins' ),
			'param_name' => 'show_category',
			'value' => array( 
				esc_html__( 'Yes', 'amz-composer-plugins' ) => 'yes', 
				esc_html__( 'No', 'amz-composer-plugins' ) => 'no' 
			),
			'description' => esc_html__( 'Do you want to show category/terms', 'amz-composer-plugins' )
		),
...
// Vertical Post
vc_map( array(
	'name' => esc_html__( 'Vertical Post Overlay', 'amz-composer-plugins' ), //Title
	'base' => 'vertical_post_overlay',
	'class' => '',
	'icon' => 'icon-pixel8es',
	'category' => esc_html__( 'By Composer', 'amz-composer-plugins' ), 
	'params' => array(
...
// Post Tag
vc_map( array(
	"name" => esc_html__( 'Post Tag', 'amz-composer-plugins' ), //Title
	"base" => 'composer_post_tag',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(	
		...
		array(
			...
			"heading" => esc_html__( 'Title', 'amz-composer-plugins' ),
			'param_name' => 'title',
			'value' => esc_html__( 'Tag: ', 'amz-composer-plugins' ),
		),
		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Extra class', 'amz-composer-plugins' ),
...
// Related Post
vc_map( array(
	"name" => esc_html__( 'Related Post', 'amz-composer-plugins' ), //Title
	"base" => 'composer_related_post',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Blog slide style post count', 'amz-composer-plugins' ),
			'param_name' => 'slide_style_pos',
			'value' => array(
				esc_html__( '1', 'amz-composer-plugins' ) => '1',
				esc_html__( '2', 'amz-composer-plugins' ) => '2',
				esc_html__( '3', 'amz-composer-plugins' ) => '3',
				esc_html__( '4', 'amz-composer-plugins' ) => '4'
			),
			'description' => 'Set the default open post count',
			'dependency' => array( 'element' => 'style', 'value' => array( 'slide' ) ),
		),
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Alignment', 'amz-composer-plugins' ),
			'param_name' => 'align',
			'value' => array(
				esc_html__( 'Align Left', 'amz-composer-plugins' ) => 'text-left', 
				esc_html__( 'Align Center', 'amz-composer-plugins' ) => 'text-center', 
				esc_html__( 'Align Right', 'amz-composer-plugins' ) => 'text-right'
			),
			'description' => esc_html__( 'Select alignment.', 'amz-composer-plugins' )
		),

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'No. of Recentblog', 'amz-composer-plugins' ),
			'param_name' => 'no_of_items',
			'value' => 2,
			'description' => esc_html__( 'Enter the number of Posts you want to display (only numbers), Enter -1 for all posts', 'amz-composer-plugins' )
		),
		...
		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Content length', 'amz-composer-plugins' ),
			'param_name' => 'excerpt_length',
			'value' => '90',
			'description' => esc_html__( 'Enter excerpt length. Example: 180 (Leave it empty to apply default)', 'amz-composer-plugins' )
		),
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Number of Columns', 'amz-composer-plugins' ),
			'param_name' => 'columns',
			'value' => array(
				esc_html__( '1 Columns', 'amz-composer-plugins' ) => 'col1',
				esc_html__( '2 Columns', 'amz-composer-plugins' ) => 'col2',
				esc_html__( '3 Columns', 'amz-composer-plugins' ) => 'col3',
				esc_html__( '4 Columns', 'amz-composer-plugins' ) => 'col4'
			),
			'description' => ''
		),
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Above Image', 'amz-composer-plugins' ),
			'param_name' => 'above_image_meta',
			'value' => array(
				esc_html__( 'None', 'amz-composer-plugins' ) => 'none',
				esc_html__( 'Date', 'amz-composer-plugins' ) => 'date',
				esc_html__( 'Category', 'amz-composer-plugins' ) => 'category',
				esc_html__( 'Author', 'amz-composer-plugins' ) => 'author'
			),
			'description' => ''
		),
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Top Meta', 'amz-composer-plugins' ),
			'param_name' => 'top_meta',
			'value' => array(
				esc_html__( 'Date', 'amz-composer-plugins' ) => 'date',
				esc_html__( 'Category', 'amz-composer-plugins' ) => 'category',
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Bottom Meta', 'amz-composer-plugins' ),
			'param_name' => 'bottom_meta',
			'value' => array(
				esc_html__( 'Like and Comment', 'amz-composer-plugins' ) => 'like_comment',
				esc_html__( 'Link', 'amz-composer-plugins' ) => 'link',
				esc_html__( 'None', 'amz-composer-plugins' ) => 'none'
			),
			'description' => ''
		),
		...
		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Show Like', 'amz-composer-plugins' ),
			'param_name' => 'show_like',
			'value' => array(
				esc_html__( 'Yes', 'amz-composer-plugins' ) => 'yes',  
				esc_html__( 'No', 'amz-composer-plugins' ) => 'no' 
			),
			'description' => esc_html__( 'Do you like to display "Like" in post?', 'amz-composer-plugins' ),
			'dependency' => array( 'element' => 'bottom_meta', 'value' => array( 'like_comment' ) )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Show Comments', 'amz-composer-plugins' ),
			'param_name' => 'show_comment',
			'value' => array(
				esc_html__( 'Yes', 'amz-composer-plugins' ) => 'yes',  
				esc_html__( 'No', 'amz-composer-plugins' ) => 'no' 
			),
			'description' => esc_html__( 'Do you like to display comments number in post', 'amz-composer-plugins' ),
			'dependency' => array( 'element' => 'bottom_meta', 'value' => array( 'like_comment' ) )
		),

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Link Text', 'amz-composer-plugins' ),
			'param_name' => 'link_text',
			'value' => esc_html__( 'Read More', 'amz-composer-plugins' ),
			'description' => esc_html__( 'Enter the link text', 'amz-composer-plugins' ),
			'dependency' => array( 'element' => 'bottom_meta', 'value' => array( 'link' ) )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Show Featured Image', 'amz-composer-plugins' ),
			'param_name' => 'show_featured_image',
			'value' => array(
				esc_html__( 'Yes', 'amz-composer-plugins' ) => 'yes',  
				esc_html__( 'No', 'amz-composer-plugins' ) => 'no'
			),
			'description' => esc_html__( 'Do you like to display featured image in post', 'amz-composer-plugins' )
		),

		array(
			'type' => 'dropdown',
			'heading' => esc_html__( 'Show Description', 'amz-composer-plugins' ),
			'param_name' => 'show_description',
			'value' => array(
				esc_html__( 'Yes', 'amz-composer-plugins' ) => 'yes',  
				esc_html__( 'No', 'amz-composer-plugins' ) => 'no'
			),
			'description' => esc_html__( 'Do you like to display description in post', 'amz-composer-plugins' )
		),
...
// Comment Template
vc_map( array(
	"name" => esc_html__( 'Comments Template', 'amz-composer-plugins' ), //Title
	"base" => 'composer_comments_template',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
...
$sidebar_list = array();
$sidebar_list[ esc_html__( 'Choose Registered Sidebar', 'amz-composer-plugins' ) ] = '0';
foreach( $sidebars as $sidebar ) {
	$sidebar_name = $sidebar['name'];
	$sidebar_id = $sidebar['id'];

	$sidebar_list[ $sidebar_name ] = $sidebar_id;
}

// Widget Area
vc_map( array(
	"name" => esc_html__( 'Widget Area', 'amz-composer-plugins' ), //Title
	"base" => 'composer_widget_area',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Widget area', 'amz-composer-plugins' ),
			'param_name'  => 'sidebar_name',
			'value'       => $sidebar_list,
			'description' => esc_html__( 'Select registered widget area?', 'amz-composer-plugins' )
		),
...
// Author Image
vc_map( array(
	"name" => esc_html__( 'Author Image', 'amz-composer-plugins' ), //Title
	"base" => 'composer_author_image',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Type', 'amz-composer-plugins' ),
			'param_name'  => 'type',
			'value'       => array(
				esc_html__( 'Round', 'amz-composer-plugins' ) => 'round',
				esc_html__( 'Square', 'amz-composer-plugins' ) => 'square'
			),
			'description' => esc_html__( 'Select image type?', 'amz-composer-plugins' )
		),

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Image Dimension', 'amz-composer-plugins' ),
			'param_name' => 'dimension',
			'value' => '',
			'description' => esc_html__( 'Type the image dimension', 'amz-composer-plugins' )
		),
...
// Post Title
vc_map( array(
	"name" => esc_html__( 'Post Title', 'amz-composer-plugins' ), //Title
	"base" => 'composer_post_title',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Tag', 'amz-composer-plugins' ),
			'param_name'  => 'type',
			'value'       => array(
				esc_html__( 'H2', 'amz-composer-plugins' ) => 'h2',
				esc_html__( 'H1', 'amz-composer-plugins' ) => 'h1',
				esc_html__( 'H3', 'amz-composer-plugins' ) => 'h3',
				esc_html__( 'H4', 'amz-composer-plugins' ) => 'h4',
				esc_html__( 'H5', 'amz-composer-plugins' ) => 'h5',
				esc_html__( 'H6', 'amz-composer-plugins' ) => 'h6',
				esc_html__( 'P', 'amz-composer-plugins' ) => 'p'
			),
			'description' => esc_html__( 'Select Title tag?', 'amz-composer-plugins' )
		),
...
// Category List
vc_map( array(
	"name" => esc_html__( 'Category List', 'amz-composer-plugins' ), //Title
	"base" => 'composer_category_list',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			...
			'param_name'  => 'type',
			'value'       => array(
				esc_html__( 'Single', 'amz-composer-plugins' ) => 'single',
				esc_html__( 'Multiple', 'amz-composer-plugins' ) => 'multiple'
			),
			'description' => esc_html__( 'Select type?', 'amz-composer-plugins' )
		),

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Prefix Text', 'amz-composer-plugins' ),
			'param_name' => 'prefix_text',
			'value' => '',
			'description' => esc_html__( 'Type the prefix text', 'amz-composer-plugins' )
		),
...
// Featured Image
vc_map( array(
	"name" => esc_html__( 'Featured Image', 'amz-composer-plugins' ), //Title
	"base" => 'composer_feature_image',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Width', 'amz-composer-plugins' ),
			'param_name' => 'width',
			'value' => '1200',
			'description' => esc_html__( 'Type the image width in integer.', 'amz-composer-plugins' )
		),

		array(
			'type' => 'textfield',
			'heading' => esc_html__( 'Height', 'amz-composer-plugins' ),
			'param_name' => 'height',
			'value' => '400',
			'description' => esc_html__( 'Type the image height in integer.', 'amz-composer-plugins' )
		),
...
// Post Date
vc_map( array(
	"name" => esc_html__( 'Post Date', 'amz-composer-plugins' ), //Title
	"base" => 'composer_post_date',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
...
// Post Author Name
vc_map( array(
	"name" => esc_html__( 'Post Author Name', 'amz-composer-plugins' ), //Title
	"base" => 'composer_post_author_name',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
...
// Post Share
vc_map( array(
	"name" => esc_html__( 'Post Share', 'amz-composer-plugins' ), //Title
	"base" => 'composer_post_share',
	"class" => '',
	"icon" => 'icon-pixel8es',
	"category" => esc_html__( 'Single Blog', 'amz-composer-plugins' ), 
	"params" => array(
		...
		array(
			'type' => 'checkbox',
			'heading' => esc_html__( 'Share Options', 'amz-composer-plugins' ),
			'param_name' => 'share',
			'value' => array(
				esc_html__( 'Facebook', 'amz-composer-plugins' )    => 'facebook',
				esc_html__( 'Twitter', 'amz-composer-plugins' )     => 'twitter',
				esc_html__( 'Google Plus', 'amz-composer-plugins' ) => 'gplus',
				esc_html__( 'Linkedin', 'amz-composer-plugins' )    => 'linkedin',
				esc_html__( 'Pinterest', 'amz-composer-plugins' )   => 'pinterest'
			),
			'description' => esc_html__( 'Please choose the share options', 'amz-composer-plugins' )
		),
