...
/**********************************************************************************************************************/
		...
		walker.callbackGetEmptyRepeatableVariation = function() {
			_.output += '<li class="ff-repeatable-item ff-repeatable-item-js ff-repeatable-item-empty-add">' +
				'<div class="ff-repeatable-add-below ff-repeatable-add-below-js" title="Add Item">ADD ITEM</div>' +
...
/**********************************************************************************************************************/
		...
		_.printOptionColorPickerWithLib = function( item, id, nameRoute ) {
            ...
            // if( item.value.indexOf('lib-') != -1 ) {
            //     color = window.vent.o.dataManager.getColorFromLibrary( item.value.replace('lib-', '') );
            // }


            var input = '';
            input +='<div class="fftm__option-type--color-picker fftm__option-type__can-reset">';
                input +='<div class="fftm__option-type--color-picker__select">';
                    input +='<div class="fftm__option-type--color-picker__select-preview-color" style="background:'+ backgroundColor +'"></div>';
                    input += hiddenInput;
        ...
        _.printOptionText = function( item, id, nameRoute ) {
            ...
            input += ' class="' + _.getItemClassesString( item ) + ' ' + shortClass + ' " ';
		...
		_.printOptionTinyMCE = function( item, id, nameRoute ) {
			...
			input += ' class="' + _.getItemClassesString( item ) + ' ff-options__textarea ff-options__tinymce"';
			input += ' data-default-value="' + item.defaultValue + '" ';
			input += ' ' + _.getPlaceholder( item ) + ' ';
			input += '>';
