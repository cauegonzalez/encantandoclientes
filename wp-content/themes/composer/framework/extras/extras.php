...
function composer_body_classes( $classes ) {
	...
	elseif( composer_is_single_shop() ){
		...
		if( is_home() || is_singular( 'post' ) || is_archive() || is_search() ) {
            //Sidebar position
            if( is_singular( 'post' ) ) {
            	$sidebar_position = composer_get_meta_value( $id, '_amz_layout', 'default', $prefix.'sidebar', 'right-sidebar' );
            }
            else {
	...
	$load_like = composer_get_option_value( 'load_like', 'php' );
	if( $load_like === 'ajax' ){
		$classes[] = 'like-count-on-ajax';
	}
